...
\begin{multicols}{3}
    ...
    \begin{Figure}
        \centering
        \includegraphics[height=3.5cm]{figures/linear-to-convnet.pdf}
    \end{Figure}
    \columnbreak
    \textbf{C}
    \begin{Figure}
        \centering
        \includegraphics[height=3.5cm]{figures/convnet-schematic.pdf}
    \end{Figure}
\end{multicols}
...
Model performance was quantified as the coefficient of determination, $R^2$

$R^2$ was modeled as: $\alpha - (\alpha - \beta) e^{-\frac{x - x_{min}}{\kappa}}$, where $x$ is the number of training samples, $\alpha$ is $R^2$ at the maximal number of training samples, $\beta$ is $R^2$ at the smallest number of training samples ($x_min$) and $\kappa$ is a free parameter that denotes that number of training samples required to achieve $R^2$ that is 67\% of the difference between $\beta$ and $\alpha$.
...
\headerbox{Conclusion and future work}{name=conclusion, column=1, span=2, below=results}{
\begin{multicols}{2}
\begin{itemize}
...
\item Differences can be much more important in some cases (see poster \# XXX)
\end{itemize}
\end{multicols}
}
...
\headerbox{References}{name=references,column=0,span=2,below=methods,above=bottom}{
...
\headerbox{Acknowledgments}{name=acknowledgments, column=2, below=methods,above=bottom}{
